...
let remainingTime = 0;
let hm = 70;
let masterLoadedFromFile = false;
...
// ============================================================
// RICH OUTPUT BUILDER
// Walks text2 DOM and replaces each word with its colour span.
// Uses queue.shift() to stay in sync with resultMap word order.
// Red words (missing from typed) are skipped in the DOM walk
// since they don't exist in text2 — they only appear in the
// legacy L.output string, not in buildRichOutput.
// ============================================================
function buildRichOutput(editorDiv, resultMap) {
    const queue = [...resultMap];
    ...
    function processNode(node) {
        ...
        if (node.nodeType === 1) {
            ...
            const isBlock  = ['p', 'div', 'h1', 'h2', 'h3', 'h4'].includes(tag);
            const isInline = ['b', 'strong', 'i', 'em', 'u', 'span'].includes(tag);

            let inner = '';
            node.childNodes.forEach(child => { inner += processNode(child); });

            if (isBlock)  return `<${tag}${style ? ` style="${style}"` : ''}>${inner}</${tag}>`;
            if (isInline) return `<${tag}>${inner}</${tag}>`;
            return inner;
        }

        if (node.nodeType === 3) {
            return node.textContent.replace(/(\S+)/g, (word) => {
                // Skip past any 'red' entries (missing words not in text2)
                while (queue.length && queue[0].status === 'red') queue.shift();
                const entry = queue.shift();
                if (!entry) return `<span>${word}</span>`;

                if (entry.status === 'correct') {
                    return `<span>${word}</span>`;
                } else if (entry.status === 'blue') {
                    return `<span class="blue">${word}<span class="green">{${entry.hint}}</span></span>`;
                } else if (entry.status === 'orange') {
                    return `<span class="red orange">${word}</span>`;
                }
                return `<span>${word}</span>`;
            });
        }
    ...
    let html = '';
    editorDiv.childNodes.forEach(child => { html += processNode(child); });
    return html;
}
...
// ============================================================
// LCS — CORE COMPARISON
// text1 = typed words, text2 = master words
// fmt1  = typed formats, fmt2 = master formats (null = skip)
// ============================================================
function lcs(text1, text2, fmt1, fmt2) {
    ...
    function constructLCS(i, j) {
        ...
        if (i === 0) {
            // Master word not typed — red, no typed word to show in DOM
            redWords.push(text2[j - 1]);
            resultMap.unshift({ word: text2[j - 1], status: 'red', hint: '' });
            return '<span class="red">' + text2[j - 1] + '</span> ' + constructLCS(i, j - 1);
        }

        if (j === 0) {
            // Typed word not in master — orange
            orangeWords.push(text1[i - 1]);
            resultMap.unshift({ word: text1[i - 1], status: 'orange', hint: '' });
        ...
        if (text1[i - 1] === text2[j - 1]) {
            if (checkFormats) {
                ...
                if (!formatsMatch(mFmt, tFmt)) {
                    ...
                    resultMap.unshift({ word: text1[i - 1], status: 'blue', hint: expected });
                    return constructLCS(i - 1, j - 1)
                        + `<span class="blue">${text1[i - 1]}<span class="green">{${expected}}</span></span> `;
                }
            }
            resultMap.unshift({ word: text1[i - 1], status: 'correct', hint: '' });
            return constructLCS(i - 1, j - 1) + '<span>' + text1[i - 1] + '</span> ';

        } else if (ld(text1[i - 1], text2[j - 1]).similarityPercentage >= hm &&
                   ld(text1[i - 1], text2[j - 1]).similarityPercentage < 100) {
            blueWords.push(text1[i - 1]);
            resultMap.unshift({ word: text1[i - 1], status: 'blue', hint: text2[j - 1] });
            return constructLCS(i - 1, j - 1)
    ...
    return { output, redWords, orangeWords, blueWords, resultMap };
}
...
submit.addEventListener('click', function () {
    ...
    output.innerHTML = buildRichOutput(text2, L.resultMap);
